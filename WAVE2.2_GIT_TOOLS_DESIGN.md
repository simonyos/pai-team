...
### G5 — `/commit-push-pr` (needs G4; readiness re-verified 2026-07-07)
...
- **Critic:** explicit `gh` **not-installed** and **unauthenticated** (`gh auth status`) preflight, degrading to a clear distinct error (≠ "no PR"); force non-interactive (`GH_PROMPT_DISABLED`, `GIT_TERMINAL_PROMPT=0`); optional `--reviewer` passthrough for parity.
- **A ready-made style template exists for the gh preflight** — `interactive-mode.ts:~5362` (`handleShareCommand`, backing `/share`) already does the exact `gh`-not-installed / `gh`-not-authenticated check via `spawnSync("gh", ["auth", "status"], ...)`, with the error-message wording G5 should match. `GIT_TERMINAL_PROMPT=0` also has a precedent (`package-manager.ts:~1604`, background git-remote calls during install) — match its style. `GH_PROMPT_DISABLED` has zero existing occurrences in the codebase — genuinely new for G5.
- **CORRECTION — "resolve a linked worktree back to its main repo" is NOT an existing G1 helper to reuse; it must be newly built.** `core/git/paths.ts` has no function that returns a linked worktree's main-repo directory — `getGitRoot`/`findGitPaths` deliberately return the *current* worktree's own root. G5 needs new code here (e.g. derive `dirname(commonGitDir)` from `findGitPaths`'s existing `commonGitDir` resolution, or shell out to `git worktree list --porcelain` and take the first entry) — do not scope this as "wire up G1's existing resolver."
...
- **D6 — fetch/pull scope — STILL OPEN, narrowed 2026-07-07:** confirmed `git fetch`/`git pull` genuinely still classify as mutating (prompt) today (`safelist.ts`/`policy.ts` have no read-only case for either verb). Accept the prompt for G5, or seed a project-level allow? The gh-read-scope half of this decision is **moot — drop it**: `gh pr view`/`list`/`diff`/`checks`/`status` (and other resources' listed read actions) are *already* auto-allowed today via the existing nuanced `ghReadOnly()` classifier (`policy.ts`, `GH_RESOURCE_READ_ACTIONS`) — there is no rule left to seed for that half.
