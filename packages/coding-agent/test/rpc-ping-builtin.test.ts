...
// Exercises the coded (built-in) command registration path in RPC mode via the
// `ping_builtin` stub command (issue XZI-6).
...
describe("RPC coded-command dispatch (ping_builtin)", () => {
	afterEach(() => {
		rpcIo.outputLines = [];
		rpcIo.lineHandler = undefined;
	});

	test("dispatches ping_builtin and returns a success response", async () => {
		...
		try {
			...
			rpcIo.lineHandler?.(JSON.stringify({ id: "ping-1", type: "ping_builtin" }));

			await vi.waitFor(() => {
				expect(parseOutputLines()).toContainEqual({
					id: "ping-1",
					type: "response",
					command: "ping_builtin",
					success: true,
					data: { pong: true },
				});
			});
